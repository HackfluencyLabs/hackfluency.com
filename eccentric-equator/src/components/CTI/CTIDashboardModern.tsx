...
import { THEMES as GLOBAL_THEMES, type ThemeConfig as GlobalThemeConfig } from '../../lib/themes';
...
// ═══════════════════════════════════════════════════════════════════════════════
// THEME SYSTEM — Uses global theme from src/lib/themes.ts
// CTI-specific adapter: maps GlobalThemeConfig → local "theme" object
// ═══════════════════════════════════════════════════════════════════════════════

/** CTI-local theme shape (extends global with nodeRootColor) */
interface CTITheme extends GlobalThemeConfig {
  nodeRootColor: string;
}

/** Convert a global theme to CTI's expected shape */
function toCTITheme(t: GlobalThemeConfig): CTITheme {
  return {
    ...t,
    nodeRootColor: t.accent,
  };
}
...
interface CTINodeData {
  ...
  parentId?: string;
  t?: (key: string) => string;
  /** True when a light theme is active — drives contrast-aware styling in CTINode */
  isLight?: boolean;
}
...
const CTINode = ({ data }: { data: CTINodeData }) => {
  ...
  const isLight = data.isLight ?? false;

  if (isExpanded) {
    // Light themes: solid opaque card with colored accent — no transparency bleed onto white canvas
    // Dark themes: keep the atmospheric translucent glow gradient
    const expandedBg = isLight
      ? '#ffffff'
      : `linear-gradient(180deg, ${style.bg}dd 0%, ${style.bg}99 100%)`;
    const expandedBorder = isLight
      ? `1px solid ${style.bg}55`
      : '2px solid rgba(255,255,255,0.4)';
    const expandedShadow = isLight
      ? `0 6px 32px rgba(0,0,0,0.14), 0 0 0 2px ${style.bg}30, inset 0 0 0 3px ${style.bg}18`
      : `0 0 40px ${style.glow}, 0 0 80px ${style.glow}, 0 20px 60px rgba(0,0,0,0.5)`;
    const headerColor = isLight ? style.bg : '#fff';
    const headerDivider = isLight ? `1px solid ${style.bg}35` : '1px solid rgba(255,255,255,0.2)';
    const bodyColor = isLight ? '#1a1a1a' : '#fff';
    const infoBg = isLight ? `${style.bg}0d` : 'rgba(0,0,0,0.3)';
    const sectionLabelColor = isLight ? `${style.bg}cc` : 'rgba(255,255,255,0.6)';
    const pillTextColor = isLight ? style.bg : '#fff';
    const linkBg = isLight ? `${style.bg}10` : 'rgba(59, 130, 246, 0.12)';
    const linkBorder = isLight ? `${style.bg}40` : 'rgba(59, 130, 246, 0.3)';
    const linkColor = isLight ? style.bg : '#93C5FD';
    const linkBgHover = isLight ? `${style.bg}22` : 'rgba(59, 130, 246, 0.25)';
    const linkBorderHover = isLight ? `${style.bg}70` : 'rgba(59, 130, 246, 0.6)';
    ...
    return (
      <div
        ...
        style={{
          ...
          ...(isLight ? { borderLeft: `4px solid ${style.bg}` } : {}),
      ...
      >
        <Handle type="target" position={Position.Left} style={{ background: isLight ? style.bg : '#fff', width: 10, height: 10 }} />
        ...
        {data.info && (
          <div style={{
            ...
            fontSize: '11px',
            lineHeight: 1.5,
            color: isLight ? '#333' : 'var(--cti-text-secondary)',
            whiteSpace: 'pre-line',
            maxHeight: '80px',
            overflow: 'auto',
        ...
        {data.connections && data.connections.length > 0 && (
          <div style={{ width: '100%' }}>
            <div style={{ fontSize: '9px', color: sectionLabelColor, marginBottom: '6px', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.8px' }}>
        ...
        {data.links && data.links.length > 0 && (
          <div style={{ width: '100%', marginTop: '10px' }}>
            <div style={{ fontSize: '9px', color: sectionLabelColor, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.8px', fontWeight: 700 }}>
              {t('dashboard.sources')}
            </div>
              ...
              {data.links.map((link, i) => (
                <a
                  ...
                  onMouseEnter={(e) => {
                    ...
                    (e.currentTarget as HTMLElement).style.borderColor = linkBorderHover;
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.background = linkBg;
                    (e.currentTarget as HTMLElement).style.borderColor = linkBorder;
                  }}
                >
        ...
        <Handle type="source" position={Position.Right} style={{ background: isLight ? style.bg : '#fff', width: 10, height: 10 }} />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      style={{
        background: style.bg,
        boxShadow: isLight
          ? `0 2px 12px ${style.glow}, 0 0 0 3px ${style.bg}35`
        ...
        color: '#fff',
        fontSize: baseSize > 80 ? '12px' : '10px',
        fontWeight: 800,
        textAlign: 'center',
        padding: '8px',
        border: isLight
    ...
    >
      <Handle type="target" position={Position.Left} style={{ background: isLight ? style.bg : '#fff', width: 8, height: 8 }} />
      <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '90%', lineHeight: 1.2 }}>
        {data.label}
      </span>
      {data.expandable && (data.childCount ?? 0) > 0 && (
        <div style={{
          position: 'absolute',
          top: -4,
          right: -4,
          background: 'var(--cti-accent)',
          color: 'var(--cti-text-on-accent)',
          borderRadius: '10px',
          padding: '2px 6px',
          fontSize: '9px',
          fontWeight: 700,
          boxShadow: '0 2px 4px rgba(0,0,0,0.3)',
          border: '2px solid var(--cti-bg)',
        }}>
          +{data.childCount}
        </div>
      )}
      <Handle type="source" position={Position.Right} style={{ background: isLight ? style.bg : '#fff', width: 8, height: 8 }} />
    </div>
  );
};
...
// ═══════════════════════════════════════════════════════════════════════════════
// REUSABLE STYLE CONSTANTS (extracted to reduce redundancy)
// ═══════════════════════════════════════════════════════════════════════════════

const commonFlex = { display: 'flex' as const, alignItems: 'center' as const };
const makeCommonCard = (th: CTITheme) => ({ padding: '24px', background: th.bgCard, borderRadius: '14px', border: `1px solid ${th.borderPrimary}` });
const commonGradientCard = (color: string) => ({
  background: `linear-gradient(135deg, ${color}10, transparent)`,
  border: `1px solid ${color}30`,
  borderRadius: '12px',
});
const commonSectionHeader = (color: string) => ({
  display: 'flex' as const, alignItems: 'center' as const, gap: '10px', marginBottom: '18px',
});
const commonAccentBar = (color: string) => ({
  width: '4px', height: '24px', borderRadius: '2px', background: color,
});
const commonSectionTitle = (color: string) => ({
  fontSize: '14px', color, textTransform: 'uppercase' as const, letterSpacing: '2px', fontWeight: 700,
});
const commonBadge = (color: string) => ({
  padding: '4px 12px', borderRadius: '20px', background: `${color}15`, border: `1px solid ${color}60`, color, fontSize: '11px', fontWeight: 600,
});
const commonHoverLink = { transition: 'all 0.2s ease', cursor: 'pointer' };
const commonScrollableBox = { overflowY: 'auto' as const, flex: 1, paddingRight: '6px' };
...
const useResponsive = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      debounceTimer = setTimeout(() => setWindowWidth(window.innerWidth), 100);
    };
  ...
  return {
    ...
    isLargeDesktop: windowWidth >= 1400,
    windowWidth,
  };
};

// Responsive style helpers
const responsiveGrid = (isMobile: boolean, isTablet: boolean) => ({
  threeColumn: isMobile ? '1fr' : isTablet ? '1fr 1fr' : '1fr 1.5fr 1fr',
  twoColumn: isMobile ? '1fr' : '1fr 1fr',
  twoColumnUneven: isMobile ? '1fr' : '1.2fr 0.8fr',
  metricsGrid: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1fr',
  fourColumn: isMobile ? '1fr 1fr' : isTablet ? '1fr 1fr 1fr' : '1fr 1fr 1fr 1fr',
});

const responsivePanel = (isMobile: boolean, isTablet: boolean = false) => ({
  padding: isMobile ? '16px' : isTablet ? '20px 24px' : '24px 40px',
  gap: isMobile ? '12px' : '20px',
});

const responsiveCorrelationPanel = (isMobile: boolean, isTablet: boolean) => ({
  flexDirection: isMobile ? 'column' : 'row' as const,
  sidebarWidth: isMobile ? '100%' : isTablet ? '280px' : '340px',
});
...
// Main Component with Auth gate + I18n Provider wrapper
...
const CTIDashboardInner: React.FC = () => {
  ...
  const gridLayout = responsiveGrid(isMobile, isTablet);
  const panelLayout = responsivePanel(isMobile, isTablet);
  const correlationLayout = responsiveCorrelationPanel(isMobile, isTablet);
  const heightHelper = (defaultHeight?: string) => responsiveHeight(isMobile, isTablet, defaultHeight);
  
  const [data, setData] = useState<DashboardData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedNodes, setExpandedNodes] = useState<Map<string, CTINodeData>>(new Map());
  ...
  const { theme: globalTheme, themeId: activeThemeId, setThemeId: setActiveThemeId } = useTheme();
  const theme = useMemo(() => toCTITheme(globalTheme), [globalTheme]);

  const [nodes, setNodes, onNodesChange] = useNodesState<Node>([]);
  const [edges, setEdges, onEdgesChange] = useEdgesState<Edge>([]);

  // Re-generate graph when theme changes (updates root/actor node colors)
  useEffect(() => {
    if (data) generateGraph(data, t, theme);
  }, [activeThemeId]);
  ...
  const loadDashboard = async () => {
    try {
      const filename = language === 'es' ? 'private/cti-dashboard-es.json' : 'private/cti-dashboard.json';
      ...
      generateGraph(dashboardData, t, theme);
      setError(null);
    } catch (err) {
  ...
  const generateGraph = (dashboardData: DashboardData, translateFn: (key: string) => string, currentTheme: CTITheme = toCTITheme(GLOBAL_THEMES[0])) => {
    const newNodes: Node[] = [];
    const newEdges: Edge[] = [];
    ...
    // Theme-aware edge label colors (SVG attributes — must be concrete hex values, no CSS vars)
    const edgeLabelBg = currentTheme.bgCard;
    const edgeLabelFg = currentTheme.isLight ? '#444' : '#aaa';
    ...
    // Override root/actor node style colors with active theme accent
    const themeRootStyle = { bg: currentTheme.nodeRootColor, glow: currentTheme.accentGlow, light: currentTheme.accentDim };
    NODE_STYLES['actor'] = themeRootStyle;
    NODE_STYLES['root'] = { ...themeRootStyle, glow: currentTheme.accentGlow };
    ...
    newEdges.push({
      ...
      style: { stroke: currentTheme.accent, strokeWidth: socialEdgeWidth, strokeDasharray: corrStrength === 'weak' ? '8,6' : undefined },
      label: `${translateFn('dashboard.temporalProximity')} ${Math.round(temporalProximity * 100)}%`,
      labelStyle: { fill: edgeLabelFg, fontSize: 10, fontFamily: 'Space Grotesk' },
      labelBgStyle: { fill: edgeLabelBg, fillOpacity: 0.95 },
      markerEnd: { type: MarkerType.ArrowClosed, color: currentTheme.accent },
    });
    ...
    newEdges.push({
      ...
      labelStyle: { fill: edgeLabelFg, fontSize: 10, fontFamily: 'Space Grotesk' },
      labelBgStyle: { fill: edgeLabelBg, fillOpacity: 0.95 },
      markerEnd: { type: MarkerType.ArrowClosed, color: 'var(--cti-accent)' },
    });
    ...
    posts.forEach((post, i) => {
      ...
      newEdges.push({
        ...
        style: { stroke: currentTheme.accent, strokeWidth: 1.5, strokeDasharray: '3,3' },
        markerEnd: { type: MarkerType.ArrowClosed, color: currentTheme.accent },
      });
    });
    ...
    if (socialCves.length > 0 && cveOverlap > 0) {
      ...
      newEdges.push({
        ...
        labelStyle: { fill: edgeLabelFg, fontSize: 9 },
        labelBgStyle: { fill: edgeLabelBg, fillOpacity: 0.95 },
        markerEnd: { type: MarkerType.ArrowClosed, color: '#E31B23' },
      });
  ...
  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    if (isCurrentlyExpanded) {
      ...
      const newExpanded = new Map(expandedNodes);
      newExpanded.set(node.id, node.data as unknown as CTINodeData);
      setExpandedNodes(newExpanded);
  ...
  const onConnectionClick = useCallback((edgeId: string) => {
    ...
    if (edge) {
      ...
      if (targetNode) {
        const newExpanded = new Map(expandedNodes);
        newExpanded.set(targetNodeId, targetNode.data as unknown as CTINodeData);
        setExpandedNodes(newExpanded);
    ...
    return nodes
      .filter(node => {
        ...
        if (parentId && node.data?.hidden) {
          return expandedParents.has(parentId as string);
        }
        return true;
      })
      .map((node) => {
        const parentId = node.data?.parentId;
        const isChildHidden = parentId && node.data?.hidden && !expandedParents.has(parentId as string);
  ...
  const computedEdges = useMemo(() => {
    const HIGHLIGHT_MAP: Record<string, string> = {
      ...
      '#3B82F6': '#60A5FA',
      '#00D26A': '#4ADE80',
      // Dynamic theme accent highlight
      [theme.accent]: theme.accentSecondary || '#4ADE80',
    };
    ...
    return edges
      .filter(edge => {
        ...
        if (sourceParent && sourceParent !== 'infra_source' && sourceParent !== 'social_source') {
          return expandedParents.has(sourceParent as string);
        }
        if (targetParent && targetParent !== 'infra_source' && targetParent !== 'social_source') {
          return expandedParents.has(targetParent as string);
        }
        return true;
      })
      .map(edge => {
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1600px', width: '100%' }}>
        ...
        <div style={{
          ...
          background: `linear-gradient(135deg, ${theme.bgCard} 0%, ${theme.bgPrimary} 100%)`,
          borderRadius: '12px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '12px' : '16px', width: isMobile ? '100%' : 'auto' }}>
            <div style={{ position: 'relative', width: isMobile ? '80px' : '120px', height: isMobile ? '80px' : '120px', flexShrink: 0 }}>
              <svg viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)' }}>
                <circle cx="50" cy="50" r="40" fill="none" stroke="var(--cti-bg-track)" strokeWidth="8" />
                <circle
              ...
              <div style={{
                ...
                <span style={{ fontSize: isMobile ? '9px' : '11px', color: 'var(--cti-text-dim)' }}>/100</span>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '9px', color: 'var(--cti-text-dim)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>
                {t('dashboard.riskScale')}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px' }}>
                <span style={{ color: '#00D26A', fontWeight: 600 }}>●</span>
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.riskScale.low')}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px' }}>
                <span style={{ color: '#F59E0B', fontWeight: 600 }}>●</span>
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.riskScale.moderate')}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px' }}>
                <span style={{ color: '#FF6B35', fontWeight: 600 }}>●</span>
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.riskScale.elevated')}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '10px' }}>
                <span style={{ color: '#E31B23', fontWeight: 600 }}>●</span>
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.riskScale.critical')}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, width: isMobile ? '100%' : 'auto' }}>
            ...
            <div style={{
              display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '16px', fontSize: isMobile ? '11px' : '12px', color: 'var(--cti-text-muted)',
              flexWrap: 'wrap',
            }}>
            <span style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{
                  color: data!.status.trend === 'decreasing' ? '#00D26A' : data!.status.trend === 'stable' ? '#F59E0B' : '#E31B23',
                }}>
                  {data!.status.trend === 'decreasing' ? '↓' : data!.status.trend === 'stable' ? '→' : '↑'} 
                ...
                {data!.assessmentLayer?.baselineComparison && (
                  <span style={{ 
                    ...
                    background: 'var(--cti-bg)',
                    borderRadius: '4px',
                    border: '1px solid var(--cti-tab-border)',
                  }}>
        ...
        {/* Intelligence Flow Section - Compact 3 Column Layout */}
            ...
            <div style={{ 
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--cti-accent)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>
                    {t('section.socialIntelligence')}
                  </span>
                </div>
                  ...
                  {data!.socialIntel?.topPosts?.map((post, i) => {
                    ...
                    return (
                      <div key={i} style={{
                        ...
                        background: 'var(--cti-bg)', borderRadius: '10px',
                        borderLeft: '3px solid #3B82F6',
                      }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <a href={`https://x.com/${authorUsername}`} target="_blank" rel="noopener noreferrer"
                            style={{ fontSize: '12px', fontWeight: 600, color: 'var(--cti-accent)', textDecoration: 'none' }}>
                            @{authorUsername}
                          </a>
                          <span style={{ fontSize: '11px', color: 'var(--cti-text-dim)' }}>🔥 {post.engagement}</span>
                        </div>
                        <p style={{ fontSize: '12px', color: 'var(--cti-text-secondary)', lineHeight: 1.5, margin: 0 }}>
              ...
              <div style={{
                ...
                background: `linear-gradient(135deg, ${theme.bgCard}cc 0%, ${theme.bgPrimary} 100%)`,
                borderRadius: '14px',
                border: `2px solid ${theme.accentBorder}`,
                boxShadow: `0 0 50px ${theme.accentDim}, inset 0 0 40px ${theme.accentDim}`,
                position: 'relative',
                height: 'auto',
                order: isMobile ? '-1' : '0',
              }}>
                <style>{`
                  ...
                  .narrative-border {
                    ...
                    background: repeating-linear-gradient(
                      ...
                      transparent,
                      transparent 4px,
                      var(--cti-accent-glow, rgba(0, 210, 106, 0.6)) 4px,
                      var(--cti-accent-glow, rgba(0, 210, 106, 0.6)) 8px
                    );
                  ...
                  @supports not (mask-composite: exclude) {
                    .narrative-border {
                      background: linear-gradient(135deg, var(--cti-accent-dim, rgba(0,210,106,0.3)), transparent);
                      animation: none;
                    }
                  }
                `}</style>
                ...
                <div style={{
                  boxShadow: `0 0 40px ${theme.accentGlow}, inset 0 0 30px ${theme.accentDim}`,
                  borderRadius: '10px',
                  padding: isMobile ? '16px' : '24px',
                  background: `${theme.bgPrimary}b3`,
                  position: 'relative',
                  zIndex: 1,
                }}>
                  <div style={{
                    ...
                    <div style={{
                      width: '6px', height: '28px', borderRadius: '3px', background: theme.accent,
                      boxShadow: `0 0 10px ${theme.accent}`,
                    }} />
                    <span style={{
                      fontSize: '15px', color: theme.accent, textTransform: 'uppercase',
                  ...
                  <p style={{
                    fontSize: '17px', color: 'var(--cti-text-primary)', lineHeight: 1.9,
              ...
              {data!.executive.recommendedActions && data!.executive.recommendedActions.length > 0 && (
                <div style={{
                  padding: isMobile ? '16px' : '20px',
                  background: `linear-gradient(135deg, ${theme.bgCard} 0%, ${theme.bgTertiary} 100%)`,
                  borderRadius: '12px',
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: '16px' }}>⚡</span>
                    <span style={{ fontSize: '12px', color: '#F59E0B', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>
                      {t('dashboard.recommendedActions')}
                    </span>
                  </div>
                  <div style={{ overflowY: 'auto', overflowX: 'hidden', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px', paddingRight: '6px' }}>
                    {data!.executive.recommendedActions.map((action, i) => (
                      <div
                        ...
                        onMouseEnter={(e) => {
                          ...
                          el.style.zIndex = '100';
                          el.style.background = `linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, ${theme.bgCard} 100%)`;
                          el.style.borderColor = 'rgba(245, 158, 11, 0.6)';
                          el.style.boxShadow = '0 8px 32px rgba(245, 158, 11, 0.3), 0 0 40px rgba(245, 158, 11, 0.15)';
                          el.style.transform = 'translateX(8px) scale(1.02)';
                          el.style.padding = '16px 18px';
                          const textEl = el.querySelector('.action-text') as HTMLElement;
                          if (textEl) {
                            textEl.style.maxHeight = '800px';
                            textEl.style.opacity = '1';
                          }
                          const arrowEl = el.querySelector('.action-arrow') as HTMLElement;
                          if (arrowEl) {
                        ...
                        onMouseLeave={(e) => {
                          const el = e.currentTarget;
                          el.style.zIndex = '1';
                          el.style.background = 'var(--cti-bg)';
                          ...
                          if (arrowEl) {
                            ...
                            arrowEl.style.transform = 'translateX(-10px)';
                          }
                        }}
                      >
        ...
        {/* Key Findings + Quick Metrics Row - Compact */}
        <div style={{
          display: 'grid', gridTemplateColumns: gridLayout.twoColumnUneven, gap: panelLayout.gap,
        }}>
          {/* Key Findings */}
          <div style={{
            padding: '24px', background: 'var(--cti-bg-card)', borderRadius: '14px',
            border: '1px solid var(--cti-border)',
          }}>
            <div style={{
              ...
              <div style={{
                width: '4px', height: '24px', borderRadius: '2px', background: '#8B5CF6',
              }} />
              <span style={{
                fontSize: '14px', color: '#8B5CF6', textTransform: 'uppercase',
                letterSpacing: '2px', fontWeight: 700,
              }}>
            ...
            {data!.executive.keyFindings?.map((finding, i) => (
              <div key={i} style={{
                ...
                marginBottom: '14px', fontSize: '14px', color: 'var(--cti-text-secondary)', lineHeight: 1.6,
              }}>
                <span style={{ color: '#8B5CF6', flexShrink: 0, marginTop: '4px', fontSize: '16px' }}>▸</span>
                <span>{finding}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                width: '4px', height: '24px', borderRadius: '2px', background: '#3B82F6',
              }} />
              <span style={{
                fontSize: '14px', color: 'var(--cti-accent)', textTransform: 'uppercase',
                letterSpacing: '2px', fontWeight: 700,
              }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr', gap: isMobile ? '8px' : '12px' }}>
              ...
              <MetricBox label={t('metrics.medium')} value={data!.metrics.mediumCount} color="#F59E0B" isMobile={isMobile} />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            <div style={{
              ...
              height: '40px',
              borderRadius: '10px',
              background: `linear-gradient(135deg, ${theme.accent} 0%, ${theme.nodeRootColor}cc 100%)`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            ...
            <div>
              <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--cti-text-primary)', fontFamily: 'Space Grotesk' }}>
                {t('subscription.title')}
              </div>
              <div style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>
          ...
          <form onSubmit={async (e) => {
            ...
            if (msgEl) {
              ...
              msgEl.style.color = result.success ? theme.accent : '#ff453a';
            }
  ...
  const renderCorrelationPanel = () => {
    ...
    const strengthColor = corrStrength === 'strong' ? theme.accent : corrStrength === 'moderate' ? '#F59E0B' : '#E31B23';

    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: correlationLayout.flexDirection as React.CSSProperties['flexDirection'], position: 'relative', overflow: 'hidden', minHeight: 0 }}>
        {/* Left: Correlation Methodology Panel */}
        <div style={{
          width: correlationLayout.sidebarWidth, flexShrink: 0, overflowY: 'auto', overflowX: 'hidden',
          background: theme.bgTertiary, borderRight: isMobile ? 'none' : `1px solid ${theme.borderPrimary}`,
          borderBottom: isMobile ? `1px solid ${theme.borderPrimary}` : 'none',
          padding: isMobile ? '16px' : isTablet ? '16px' : '20px', display: 'flex', flexDirection: 'column', gap: isMobile ? '12px' : '16px',
          minHeight: 0, maxHeight: isMobile ? '38vh' : '100%',
        }}>
          ...
          <div style={{
            ...
            <div style={{
              fontSize: isMobile ? '9px' : '10px', color: 'var(--cti-text-dim)', marginTop: '8px', lineHeight: 1.5,
            }}>
          ...
          <div style={{
            ...
            <div style={{
              fontSize: '10px', color: 'var(--cti-text-muted)', textTransform: 'uppercase',
            ...
            {[
              ...
              { label: t('dashboard.serviceMatch'), value: factors?.serviceMatch ?? 0, desc: t('dashboard.factor.serviceMatch.desc'), color: 'var(--cti-accent)' },
              { label: t('dashboard.temporalProximity'), value: factors?.temporalProximity ?? 0, desc: t('dashboard.factor.temporalProximity.desc'), color: theme.accent },
              ...
              <div key={i} style={{ marginBottom: '12px' }}>
                <div style={{
                  ...
                  <span style={{ fontSize: '11px', color: 'var(--cti-text-secondary)', fontWeight: 500 }}>{f.label}</span>
                  <span style={{
                ...
                <div style={{
                  height: '6px', borderRadius: '3px', background: 'var(--cti-bg-track)',
                  overflow: 'hidden',
                }}>
                  <div style={{
                ...
                <div style={{ fontSize: '9px', color: 'var(--cti-text-disabled)', marginTop: '2px' }}>{f.desc}</div>
              </div>
            ))}
          </div>
          ...
          {scoring && (
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: '11px', color: 'var(--cti-text-muted)' }}>{t('dashboard.computedScore')}</span>
                <span style={{
              ...
              {scoring.weights && scoring.components && Object.entries(scoring.weights).map(([key, weight]) => {
                ...
                return (
                  <div key={key} style={{
                    ...
                    <span style={{ color: 'var(--cti-text-muted)', flex: 1 }}>
                      {t('scoring.' + key) || key.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase())}
                    </span>
                    <span style={{ color: 'var(--cti-text-dim)', width: '40px', textAlign: 'right' }}>
                      ×{((weight as number) * 100).toFixed(0)}%
                    </span>
                    <span style={{
                      color: 'var(--cti-text-secondary)', fontWeight: 600, fontFamily: 'Space Grotesk',
              ...
              <div style={{
                ...
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.confidenceLevel')}</span>
          ...
          {classification && (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: '10px', color: 'var(--cti-text-muted)', marginTop: '2px' }}>
                  {t('status.confidence')}: {classification.confidence}%
                </div>
              </div>
              {classification.rationale && (
                <div style={{ fontSize: '10px', color: 'var(--cti-text-muted)', lineHeight: 1.5, marginBottom: '8px' }}>
                  {classification.rationale}
                </div>
              )}
              {classification.indicators?.map((ind, i) => (
                <div key={i} style={{
                  ...
                  fontSize: '10px', color: 'var(--cti-text-dim)', marginBottom: '4px',
                }}>
                  <span style={{ color: 'var(--cti-text-disabled)' }}>▸</span>
                  {ind}
                </div>
              ))}
            </div>
            ...
            { label: t('indicators.cves'), value: iocStats.uniqueCVECount, total: data!.indicators?.cves?.length || 0 },
              ...
              { label: t('dashboard.totalIocs'), value: iocStats.totalIndicators },
                  ...
                  <div key={i} style={{
                    ...
                    background: 'var(--cti-bg)', border: '1px solid var(--cti-tab-border)',
                    textAlign: 'center',
                  }}>
                    <div style={{
                    ...
                    <div style={{ fontSize: '9px', color: 'var(--cti-text-disabled)', textTransform: 'uppercase' }}>
                      {stat.label}
                    </div>
                    {stat.total !== undefined && stat.total !== stat.value && (
                      <div style={{ fontSize: '8px', color: 'var(--cti-text-disabled)', marginTop: '2px' }}>
              ...
              {iocStats.duplicationRatio !== undefined && (
                <div style={{
                  fontSize: '9px', color: 'var(--cti-text-disabled)', marginTop: '8px',
          ...
          {freshness && (
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ color: 'var(--cti-text-muted)' }}>{t('status.risk')}</span>
                <span style={{
              ...
              <div style={{
                ...
                <span 
                  style={{ color: 'var(--cti-text-muted)', cursor: 'help' }}
                ...
                >
                  ...
                  {socialAgeHovered && (
                    <div style={{
                      ...
                      <span style={{ fontSize: '10px', color: 'var(--cti-text-muted)' }}>{t('dashboard.socialAgeTooltip')}</span>
                    </div>
                  )}
                </span>
                <span style={{ color: 'var(--cti-text-secondary)', fontFamily: 'Space Grotesk' }}>{freshness.socialAgeHours}h</span>
              </div>
              <div style={{
                ...
                >
                  ...
                  {infraAgeHovered && (
                    <div style={{
                      ...
                      <span style={{ fontSize: '10px', color: 'var(--cti-text-muted)' }}>{t('dashboard.infraAgeTooltip')}</span>
                    </div>
                  )}
                </span>
                <span style={{ color: 'var(--cti-text-secondary)', fontFamily: 'Space Grotesk' }}>{freshness.infraAgeHours}h</span>
              </div>
              <div style={{
                height: '4px', borderRadius: '2px', background: 'var(--cti-bg-track)', marginTop: '8px',
              }}>
                <div style={{
        ...
        <div style={{ flex: 1, position: 'relative', minHeight: isMobile ? '350px' : 'auto' }}>
          ...
          {!isMobile && (
            <div style={{
              ...
              padding: '10px 16px',
              background: `${theme.bgCard}f2`,
              borderRadius: '8px', border: '1px solid var(--cti-border)',
              backdropFilter: 'blur(10px)',
            }}>
              <div style={{
                fontFamily: 'Space Grotesk', fontSize: '13px', fontWeight: 600, color: 'var(--cti-text-primary)',
              }}>
                ◈ {t('dashboard.correlationMapTitle')}
              </div>
              <div style={{ fontSize: '9px', color: 'var(--cti-text-dim)', marginTop: '3px' }}>
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              <LegendItem color={theme.accent} label={t('dashboard.socialIntel')} isMobile={isMobile} />
            ...
            {!isMobile && (
              <>
                <div style={{ fontSize: '9px', color: 'var(--cti-text-dim)', marginTop: '8px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', fontSize: '9px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.crossSourceMatch')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <div style={{ width: '20px', height: '2px', background: '#555', borderTop: '2px dashed #555' }} />
                    <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.noMatchFound')}</span>
                  </div>
                </div>
              </>
            )}
          ...
          >
            <Background color={theme.borderPrimary} gap={isMobile ? 20 : 30} size={1} />
            {!isMobile && (
              <Controls style={{ background: theme.bgSecondary, border: `1px solid ${theme.borderPrimary}`, borderRadius: '8px' }} />
            )}
            {!isMobile && (
              <MiniMap
                nodeColor={(node) => {
                  ...
                  const colors: Record<string, string> = {
                    ...
                    actor: theme.accent, killchain: '#F59E0B', root: theme.accent,
                  };
                  return colors[nodeType] || '#6B7280';
                }}
                style={{ background: theme.bgSecondary, border: `1px solid ${theme.borderPrimary}`, borderRadius: '8px' }}
              />
            )}
          </ReactFlow>
        </div>
  ...
  const renderDetailPanel = () => (
    <div style={{
      flex: 1, overflow: 'auto', padding: panelLayout.padding,
      display: 'flex', justifyContent: 'center',
    }}>
      <div style={{ maxWidth: '1600px', width: '100%' }}>
        {/* Risk Score + Metrics Header Row - Enhanced */}
        <div style={{
          display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '300px 1fr', gap: panelLayout.gap, marginBottom: '24px',
        }}>
          {/* Risk Gauge */}
          <div style={{
            padding: isMobile ? '16px' : '24px', background: 'var(--cti-bg-card)', borderRadius: '14px',
            border: '1px solid var(--cti-border)', display: 'flex', alignItems: 'center', gap: isMobile ? '16px' : '24px',
            flexDirection: isMobile ? 'column' : 'row',
            textAlign: isMobile ? 'center' : 'left',
          }}>
            <div style={{ position: 'relative', width: isMobile ? '80px' : '100px', height: isMobile ? '80px' : '100px' }}>
              <svg viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)' }}>
                <circle cx="50" cy="50" r="40" fill="none" stroke="var(--cti-bg-track)" strokeWidth="10" />
              ...
              <div style={{
                ...
                <span style={{ fontSize: isMobile ? '10px' : '12px', color: 'var(--cti-text-dim)' }}>/100</span>
              </div>
            </div>
            <div>
              <span style={{
          ...
          {/* Metrics Grid */}
          <div style={{
            padding: '24px', background: 'var(--cti-bg-card)', borderRadius: '14px', border: '1px solid var(--cti-border)',
          }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : gridLayout.twoColumn, gap: panelLayout.gap }}>
          ...
          <div>
            ...
            <Section title={t('section.executiveSummary')}>
              <div style={{
                ...
                boxShadow: `0 0 20px ${riskColor}12`,
              }}>
                {/* Risk Scale */}
                <div style={{
                  marginBottom: '16px', padding: '12px',
                  background: 'var(--cti-bg)', borderRadius: '8px',
                  border: '1px solid var(--cti-tab-border)',
                }}>
                  <div style={{ fontSize: '11px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '6px' }}>
                    {t('dashboard.riskScale')}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px' }}>
                    <span style={{ color: '#00D26A', fontWeight: 600 }}>●</span>
                    <span style={{ color: 'var(--cti-text-secondary)' }}>{t('dashboard.riskScale.low')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px' }}>
                    <span style={{ color: '#F59E0B', fontWeight: 600 }}>●</span>
                    <span style={{ color: 'var(--cti-text-secondary)' }}>{t('dashboard.riskScale.moderate')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px' }}>
                    <span style={{ color: '#FF6B35', fontWeight: 600 }}>●</span>
                    <span style={{ color: 'var(--cti-text-secondary)' }}>{t('dashboard.riskScale.elevated')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px' }}>
                    <span style={{ color: '#E31B23', fontWeight: 600 }}>●</span>
                    <span style={{ color: 'var(--cti-text-secondary)' }}>{t('dashboard.riskScale.critical')}</span>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{
                    fontSize: '18px', fontWeight: 700, color: riskColor, fontFamily: 'Space Grotesk',
                  }}>
                    {data!.executive.headline}
                  </div>
                </div>
                ...
                {/* Baseline Comparison */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '13px', color: 'var(--cti-text-muted)', marginBottom: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{
                      color: data!.status.trend === 'decreasing' ? '#00D26A' : data!.status.trend === 'stable' ? '#F59E0B' : '#E31B23',
                      fontWeight: 600,
                ...
                <p style={{
                  fontSize: '14px', color: 'var(--cti-text-secondary)', lineHeight: 1.8, whiteSpace: 'pre-line',
                }}>
                ...
                {data!.executive.keyFindings && data!.executive.keyFindings.length > 0 && (
                  <div style={{
                    ...
                    borderRadius: '10px', border: `1px solid ${theme.accentBorder}`,
                  }}>
                    <div style={{ fontSize: '12px', color: theme.accent, marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600 }}>
                      {t('dashboard.keyFindings')}
                    </div>
                    {data!.executive.keyFindings.map((finding, i) => (
                      <div key={i} style={{
                        fontSize: '13px', color: 'var(--cti-text-secondary)', display: 'flex',
                        alignItems: 'flex-start', gap: '10px', marginBottom: '6px', lineHeight: 1.5,
                      }}>
            ...
            <Section title={t('section.threatAnalysis')}>
              <div style={{
                padding: '20px', background: theme.bgPrimary, borderRadius: '10px',
                border: `1px solid ${theme.accentBorder}`, boxShadow: `0 0 20px ${theme.accentDim}`,
              }}>
                <div style={{ marginBottom: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('cti.killChain')}</span>
                  <div style={{
                    marginTop: '8px', marginLeft: '10px', padding: '10px 16px', background: '#F59E0B20',
                    border: '1px solid #F59E0B', borderRadius: '6px', color: '#F59E0B',
                ...
                <div style={{ marginBottom: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.correlationStrength')}</span>
                  <div style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '120px', height: '8px', background: 'var(--cti-bg-track)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{
                    ...
                    <span style={{
                      fontSize: '12px', textTransform: 'uppercase', fontWeight: 600,
                ...
                <div style={{ marginBottom: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('assessment.classification')}</span>
                  <div style={{
                    marginTop: '6px', padding: '8px 14px', background: '#8B5CF620',
                    border: '1px solid #8B5CF6', borderRadius: '6px', color: '#8B5CF6', fontSize: '13px', fontWeight: 600,
                  }}>
                    {(t('classification.type.' + (data!.assessmentLayer?.classification?.type || 'opportunistic'))).toUpperCase()}
                    <span style={{ color: 'var(--cti-text-muted)', marginLeft: '8px', fontWeight: 400 }}>
                ...
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.technicalAssessment')}</span>
                  <div style={{
                    marginTop: '10px', padding: '18px', background: 'var(--cti-bg-code)', borderRadius: '8px',
                    border: `1px solid ${theme.borderPrimary}`, fontSize: '12px', fontFamily: 'monospace',
                    color: theme.accent,
                    lineHeight: 1.8, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                    overflowWrap: 'break-word',
                  }}>
                    <div style={{ marginBottom: '12px', color: 'var(--cti-text-dim)', borderBottom: '1px solid var(--cti-tab-border)', paddingBottom: '10px', fontSize: '11px' }}>
                        ...
                        <>
                          {cleaned.split('\n\n').map((para, idx) => (
                            <div key={idx} style={{ marginBottom: '14px' }}>
                              {para.split('\n').map((line, lidx) => (
                                <div key={lidx} style={{
                                  color: line.match(/^\d+\./) ? '#F59E0B' : line.startsWith('  •') ? '#8B5CF6' : theme.accent,
                                  fontWeight: line.match(/^\d+\./) ? 600 : 400,
                                  marginBottom: line.match(/^\d+\./) ? '6px' : '4px',
                                }}>
                                  {line}
                                </div>
                              ))}
                          ...
                          {isTruncated && (
                            <div style={{
                              ...
                              fontSize: '11px', color: '#F59E0B', display: 'flex',
                    ...
                    <div style={{ marginTop: '12px', color: 'var(--cti-text-dim)', borderTop: '1px solid #1a1a1a', paddingTop: '10px', fontSize: '11px' }}>
            ...
            <Section title={t('section.socialIntelligence')}>
              {data!.socialIntel?.topPosts?.slice(0, 3).map((post, i) => {
                ...
                return (
                  <div key={i} style={{
                    padding: '16px', background: 'var(--cti-bg-card)', borderRadius: '10px',
                    border: '1px solid var(--cti-border)', marginBottom: '10px',
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      <a href={`https://x.com/${authorUsername}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '13px', fontWeight: 600, color: theme.accent, textDecoration: 'none' }}
                        onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.textDecoration = 'underline'; }}
                        onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.textDecoration = 'none'; }}
                      >
                        @{authorUsername}
                      </a>
                      <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>{post.engagement} {t('dashboard.engagement')}</span>
                    </div>
                    <p style={{ fontSize: '13px', color: 'var(--cti-text-secondary)', lineHeight: 1.6, whiteSpace: 'pre-line' }}>
                    ...
                    {post.url && (
                      <a href={post.url} target="_blank" rel="noopener noreferrer" style={{
                        ...
                        borderRadius: '6px', fontSize: '12px', color: '#93C5FD', textDecoration: 'none',
                        transition: 'all 0.2s ease',
                      }}
              ...
              {data!.socialIntel && (
                <div style={{ display: 'flex', gap: '16px', marginTop: '10px', fontSize: '12px', color: 'var(--cti-text-muted)' }}>
                  <span>{t('social.totalPosts')}: {data!.socialIntel.totalPosts}</span>
                  <span>{t('social.tone')}: <span style={{
                    color: data!.socialIntel.tone === 'confirmed' ? '#E31B23' : data!.socialIntel.tone === 'speculative' ? '#F59E0B' : '#888',
            ...
            <Section title={t('section.methodology')}>
              {data!.ctiAnalysis?.analystBrief && (
                <div style={{
                  padding: '14px', background: 'var(--cti-bg-card)', borderRadius: '8px',
                  border: '1px solid var(--cti-border)', marginBottom: '14px',
                  fontSize: '13px', color: '#bbb', lineHeight: 1.6, fontFamily: 'monospace',
                }}>
                  {data!.ctiAnalysis.analystBrief}
                </div>
              )}
              {data!.ctiAnalysis?.methodologies && data!.ctiAnalysis.methodologies.length > 0 && (
                <div style={{ marginBottom: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.analysisMethods')}</span>
                  <div style={{ marginTop: '8px' }}>
                    {data!.ctiAnalysis.methodologies.map((m, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', fontSize: '13px', color: 'var(--cti-text-secondary)' }}>
                        <span style={{ color: theme.accent, fontSize: '14px' }}>✓</span> {m}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {data!.modelMetadata && (
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.modelsUsed')}</span>
                  <div style={{ marginTop: '8px', padding: '14px', background: 'var(--cti-bg-card)', borderRadius: '8px', border: '1px solid var(--cti-border)', fontSize: '12px', fontFamily: 'monospace' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.strategic')}</span>
                      <span style={{ color: '#8B5CF6', fontWeight: 600 }}>{data!.modelMetadata.strategic}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                      <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.technical')}</span>
                      <span style={{ color: '#F59E0B', fontWeight: 600 }}>{data!.modelMetadata.technical}</span>
                    </div>
                    {data!.modelMetadata.quantization && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                        <span style={{ color: 'var(--cti-text-muted)' }}>{t('dashboard.quantization')}</span>
                        <span style={{ color: 'var(--cti-text-secondary)' }}>{data!.modelMetadata.quantization}</span>
                      </div>
                    )}
                    {data!.modelMetadata.version && (
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ color: 'var(--cti-text-muted)' }}>{t('version')}</span>
                        <span style={{ color: 'var(--cti-text-secondary)' }}>{data!.modelMetadata.version}</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div>
            ...
            <Section title={t('section.dataSources')}>
              <div style={{
                ...
                <a href="https://www.shodan.io/dashboard" target="_blank" rel="noopener noreferrer" style={{
                  padding: '14px', background: 'var(--cti-bg-card)', borderRadius: '10px',
                  border: '1px solid #3B82F630', textDecoration: 'none',
                  display: 'flex', flexDirection: 'column', gap: '6px',
                  transition: 'all 0.2s ease', cursor: 'pointer',
                }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#3B82F680'; (e.currentTarget as HTMLElement).style.background = '#3B82F612'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#3B82F630'; (e.currentTarget as HTMLElement).style.background = 'var(--cti-bg-card)'; }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '18px' }}>🔍</span>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--cti-accent)' }}>{t('dashboard.shodan')}</span>
                    <span style={{ fontSize: '11px', color: 'var(--cti-text-disabled)', marginLeft: 'auto' }}>↗</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>{data!.infrastructure?.totalHosts || 0} hosts scanned</span>
                </a>
                ...
                <a href={`https://x.com/search?q=${encodeURIComponent(data!.indicators?.keywords?.[0] || 'cybersecurity')}`} target="_blank" rel="noopener noreferrer" style={{
                  padding: '14px', background: 'var(--cti-bg-card)', borderRadius: '10px',
                  border: `1px solid ${theme.accentBorder}`, textDecoration: 'none',
                  display: 'flex', flexDirection: 'column', gap: '6px',
                  transition: 'all 0.2s ease', cursor: 'pointer',
                }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--cti-accent)'; (e.currentTarget as HTMLElement).style.background = 'var(--cti-accent-dim)'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--cti-accent-border)'; (e.currentTarget as HTMLElement).style.background = 'var(--cti-bg-card)'; }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '16px' }}>𝕏</span>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: theme.accent }}>X.com Intel</span>
                    <span style={{ fontSize: '11px', color: 'var(--cti-text-disabled)', marginLeft: 'auto' }}>↗</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>{data!.socialIntel?.totalPosts || 0} posts analyzed</span>
                </a>
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#E31B2380'; (e.currentTarget as HTMLElement).style.background = '#E31B2312'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#E31B2330'; (e.currentTarget as HTMLElement).style.background = 'var(--cti-bg-card)'; }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '18px' }}>🛡</span>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#E31B23' }}>NVD / NIST</span>
                    <span style={{ fontSize: '11px', color: 'var(--cti-text-disabled)', marginLeft: 'auto' }}>↗</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>{data!.indicators?.cves?.length || 0} CVEs referenced</span>
                </a>
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#8B5CF680'; (e.currentTarget as HTMLElement).style.background = '#8B5CF612'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#8B5CF630'; (e.currentTarget as HTMLElement).style.background = 'var(--cti-bg-card)'; }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '18px' }}>🎯</span>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#8B5CF6' }}>MITRE ATT&CK</span>
                    <span style={{ fontSize: '11px', color: 'var(--cti-text-disabled)', marginLeft: 'auto' }}>↗</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)' }}>{t('dashboard.killChain')}: {data!.ctiAnalysis?.killChainPhase || 'N/A'}</span>
                </a>
              </div>
            </Section>
            ...
            <Section title={t('section.indicators')}>
              <div style={{ marginBottom: '14px' }}>
                <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('indicators.cves')}</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
                  {data!.indicators.cves?.map((cve, i) => (
                    <a key={i} href={`https://nvd.nist.gov/vuln/detail/${cve}`} target="_blank" rel="noopener noreferrer" style={{
                      padding: '6px 10px', background: '#E31B2320', border: '1px solid #E31B23',
                      borderRadius: '6px', fontSize: '12px', color: '#E31B23', fontFamily: 'monospace',
                    ...
                    >
                      🛡 {cve} <span style={{ fontSize: '10px', opacity: 0.6 }}>↗</span>
                    </a>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: '14px' }}>
                <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.threatCategories')}</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
                  {data!.indicators.keywords?.map((kw, i) => (
                    <span key={i} style={{
                      padding: '6px 10px', background: '#8B5CF620', border: '1px solid #8B5CF6',
                      borderRadius: '6px', fontSize: '12px', color: '#8B5CF6', fontWeight: 600,
                    }}>
                      {kw}
                    </span>
                  ))}
              ...
              {data!.indicators.domains && data!.indicators.domains.length > 0 && (
                <div>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('dashboard.domains')}</span>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '8px' }}>
                    {data!.indicators.domains.map((domain, i) => (
                      <a key={i} href={`https://www.virustotal.com/gui/domain/${domain}`} target="_blank" rel="noopener noreferrer" style={{
                        padding: '6px 10px', background: '#3B82F620', border: '1px solid #3B82F6',
                        borderRadius: '6px', fontSize: '12px', color: 'var(--cti-accent)', fontFamily: 'monospace',
                        textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px',
                        transition: 'all 0.2s ease', cursor: 'pointer',
                      }}
                      ...
                      >
                        🔍 {domain} <span style={{ fontSize: '10px', opacity: 0.6 }}>↗</span>
                      </a>
                    ))}
                  </div>
                </div>
              ...
              {data!.indicators.ips && data!.indicators.ips.length > 0 && (
                <div style={{ marginTop: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>{t('indicators.ips')}</span>
                  <div style={{
                    marginTop: '8px',
                    maxHeight: '150px',
                    overflowY: 'auto',
                    ...
                    {data!.indicators.ips.map((ip, i) => (
                      <div key={i} style={{
                        ...
                        borderBottom: i < data!.indicators.ips.length - 1 ? '1px solid var(--cti-border)' : 'none',
                      }}>
                        {ip}
                      </div>
                    ))}
              ...
              {data!.infrastructure?.sampleHosts && data!.infrastructure.sampleHosts.length > 0 && (
                <div style={{ marginTop: '14px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', fontWeight: 600 }}>
                  ...
                  <div style={{
                    ...
                    {data!.infrastructure.sampleHosts.map((host, i) => (
                      <div key={i} style={{
                        padding: '10px',
                        background: i % 2 === 0 ? 'var(--cti-bg-3)' : 'transparent',
                        borderRadius: '6px',
                        marginBottom: '6px',
                        borderLeft: '3px solid #E31B23',
                      }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '13px', fontFamily: 'monospace', color: 'var(--cti-accent)', fontWeight: 700 }}>
                            {host.ip}:{host.port}
                          </span>
                          <span style={{ fontSize: '11px', color: 'var(--cti-accent)', background: '#3B82F620', padding: '4px 8px', borderRadius: '4px' }}>
                            {host.service}
                          </span>
                        </div>
                        {host.vulns && host.vulns.length > 0 && (
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                            ...
                            {host.vulns.length > 5 && (
                              <span style={{ fontSize: '11px', color: 'var(--cti-text-muted)' }}>+{host.vulns.length - 5} more</span>
                            )}
                          </div>
                        )}
                      </div>
              ...
              <Section title={t('section.infrastructure')}>
                ...
                {data!.infrastructure.exposedPorts && data!.infrastructure.exposedPorts.length > 0 && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.exposedPorts')}</span>
                    <div style={{ marginTop: '6px' }}>
                      {data!.infrastructure.exposedPorts.map((port, i) => (
                        <a key={i} href={`https://www.shodan.io/search?query=port:${port.port}`} target="_blank" rel="noopener noreferrer" style={{
                          ...
                          padding: '6px 8px', background: i % 2 === 0 ? theme.bgTertiary : 'transparent',
                        ...
                        onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = i % 2 === 0 ? theme.bgTertiary : 'transparent'; }}
                        >
                          <span style={{ color: '#F59E0B', fontFamily: 'monospace', width: '50px' }}>{port.port}</span>
                          <span style={{ color: 'var(--cti-text-secondary)', flex: 1 }}>{port.service}</span>
                          <span style={{ color: 'var(--cti-text-dim)', fontSize: '10px', marginRight: '8px' }}>{port.count} ({port.percentage}%)</span>
                ...
                {data!.infrastructure.topCountries && data!.infrastructure.topCountries.length > 0 && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.topCountries')}</span>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '6px' }}>
                      {data!.infrastructure.topCountries.map((c, i) => (
                        <span key={i} style={{
                          padding: '4px 8px', background: '#3B82F615', border: '1px solid #3B82F640',
                          borderRadius: '4px', fontSize: '10px', color: 'var(--cti-accent)', fontFamily: 'monospace',
                        }}>
                          {c.country}: {c.count}
                        </span>
                      ))}
                ...
                {data!.infrastructure.sampleHosts && data!.infrastructure.sampleHosts.length > 0 && (
                  <div>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.vulnerableHostsSample')}</span>
                    <div style={{ marginTop: '6px' }}>
                      {data!.infrastructure.sampleHosts.map((host, i) => (
                        <div key={i} style={{
                          padding: '8px', background: 'var(--cti-bg-card)', borderRadius: '4px',
                          border: '1px solid #E31B2320', marginBottom: '4px',
                          fontSize: '10px', fontFamily: 'monospace',
                        }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                            <span style={{ color: '#E31B23' }}>{host.ip}:{host.port}</span>
                            <span style={{ color: 'var(--cti-text-muted)' }}>{host.service}</span>
                          </div>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                            {host.vulns.map((v, vi) => (
            ...
            {data!.assessmentLayer && (
              <Section title={t('section.assessmentDetails')}>
                {data!.assessmentLayer.narrative && (
                  <div style={{
                    padding: '12px', background: 'var(--cti-bg)', borderRadius: '8px',
                    border: `1px solid ${theme.accentBorder}`, marginBottom: '12px',
                    fontSize: '12px', color: 'var(--cti-text-secondary)', lineHeight: 1.6, fontStyle: 'italic',
                  }}>
                    {data!.assessmentLayer.narrative}
                  </div>
                )}
                {data!.assessmentLayer.freshness && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('assessment.freshness')}</span>
                    <div style={{ marginTop: '6px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                      <div style={{ padding: '8px', background: 'var(--cti-bg-card)', borderRadius: '4px', textAlign: 'center' }}>
                        <div style={{ fontSize: '14px', fontWeight: 700, color: data!.assessmentLayer.freshness.status === 'stale' ? '#E31B23' : data!.assessmentLayer.freshness.status === 'moderate' ? '#F59E0B' : '#00D26A', fontFamily: 'Space Grotesk' }}>
                          {(data!.assessmentLayer.freshness.freshnessScore * 100).toFixed(0)}%
                        </div>
                        <div style={{ fontSize: '9px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.score')}</div>
                      </div>
                      <div style={{ padding: '8px', background: 'var(--cti-bg-card)', borderRadius: '4px', textAlign: 'center' }}>
                        <div style={{
                          fontSize: '11px', fontWeight: 600, textTransform: 'uppercase',
                          color: data!.assessmentLayer.freshness.status === 'stale' ? '#E31B23' : data!.assessmentLayer.freshness.status === 'moderate' ? '#F59E0B' : '#00D26A',
                        }}>
                          {data!.assessmentLayer.freshness.status}
                        </div>
                        <div style={{ fontSize: '9px', color: 'var(--cti-text-dim)', marginTop: '2px' }}>
                ...
                {data!.assessmentLayer.baselineComparison && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.baselineComparison')}</span>
                    <div style={{ marginTop: '6px', padding: '10px', background: 'var(--cti-bg-card)', borderRadius: '6px', border: '1px solid var(--cti-border)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--cti-text-muted)' }}>{t('dashboard.previousScore')}</span>
                        <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--cti-text-dim)', fontFamily: 'Space Grotesk' }}>
                          {data!.assessmentLayer.baselineComparison.previousRiskScore}
                        </span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--cti-text-muted)' }}>{t('dashboard.currentScore')}</span>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '11px', color: 'var(--cti-text-muted)' }}>{t('dashboard.delta')}</span>
                        <span style={{
                          fontSize: '14px', fontWeight: 700, fontFamily: 'Space Grotesk',
                          ...
                          {data!.assessmentLayer.baselineComparison.anomalyLevel && (
                            <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', marginLeft: '8px', fontWeight: 400 }}>
                              ({data!.assessmentLayer.baselineComparison.anomalyLevel})
                            </span>
                          )}
                        </span>
                ...
                {data!.assessmentLayer.classification?.rationale && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.classificationRationale')}</span>
                    <div style={{ marginTop: '6px', padding: '10px', background: 'var(--cti-bg-card)', borderRadius: '6px', border: '1px solid #8B5CF620', fontSize: '11px', color: 'var(--cti-text-secondary)', lineHeight: 1.5 }}>
                      {data!.assessmentLayer.classification.rationale}
                      {data!.assessmentLayer.classification.indicators && data!.assessmentLayer.classification.indicators.length > 0 && (
                        <div style={{ marginTop: '8px' }}>
                          {data!.assessmentLayer.classification.indicators.map((ind, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px', fontSize: '10px', color: 'var(--cti-text-muted)' }}>
                ...
                {data!.assessmentLayer.correlation?.factors && (
                  <div style={{ marginBottom: '12px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.correlationFactors')}</span>
                    <div style={{ marginTop: '6px' }}>
                      {Object.entries(data!.assessmentLayer.correlation.factors).map(([key, value], i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                          <span style={{ fontSize: '10px', color: 'var(--cti-text-muted)', width: '120px', textTransform: 'capitalize' }}>
                            {key.replace(/([A-Z])/g, ' $1').trim()}
                          </span>
                          <div style={{ flex: 1, height: '4px', background: 'var(--cti-bg-track)', borderRadius: '2px', overflow: 'hidden' }}>
                            <div style={{ width: `${(value as number) * 100}%`, height: '100%', background: (value as number) > 0.5 ? '#00D26A' : (value as number) > 0.2 ? '#F59E0B' : '#E31B23', transition: 'width 0.5s' }} />
                          </div>
                          <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', width: '35px', textAlign: 'right' }}>
                ...
                {data!.assessmentLayer.iocStats && (
                  <div>
                    <span style={{ fontSize: '10px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.indicatorStatistics')}</span>
                    <div style={{ marginTop: '6px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px' }}>
                      <div style={{ padding: '6px', background: 'var(--cti-bg-card)', borderRadius: '4px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '8px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('indicators.cves')}</div>
                      </div>
                      <div style={{ padding: '6px', background: 'var(--cti-bg-card)', borderRadius: '4px', textAlign: 'center' }}>
                        <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--cti-accent)', fontFamily: 'Space Grotesk' }}>{data!.assessmentLayer.iocStats.uniqueDomainCount}</div>
                        <div style={{ fontSize: '8px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.domains')}</div>
                      </div>
                      <div style={{ padding: '6px', background: 'var(--cti-bg-card)', borderRadius: '4px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '8px', color: 'var(--cti-text-dim)', textTransform: 'uppercase' }}>{t('dashboard.total')}</div>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div style={{
                ...
                {data!.executive.recommendedActions
                  ...
                  <div key={i} style={{
                    ...
                    background: 'var(--cti-bg)', borderRadius: '6px',
                  }}>
                    <div style={{
                    ...
                    <span style={{
                      fontSize: '12px', color: 'var(--cti-text-secondary)', lineHeight: 1.5, paddingTop: '2px',
                    }}>
                      {action.replace(/^\.\s*/, '').replace(/\*\*/g, '')}
                    </span>
                  </div>
  ...
  return (
    <div
      style={{
        ...
        overflow: 'hidden',
        // Inject CSS custom properties so child elements can reference them
        ['--cti-accent' as string]: theme.accent,
        ['--cti-accent-glow' as string]: theme.accentGlow,
        ['--cti-accent-dim' as string]: theme.accentDim,
        ['--cti-accent-border' as string]: theme.accentBorder,
        ['--cti-bg' as string]: theme.bgPrimary,
        ['--cti-bg-2' as string]: theme.bgSecondary,
        ['--cti-bg-card' as string]: theme.bgCard,
        ['--cti-bg-3' as string]: theme.bgTertiary,
        ['--cti-border' as string]: theme.borderPrimary,
        ['--cti-border-2' as string]: theme.borderSecondary,
        ['--cti-text-muted' as string]: theme.textMuted,
        ['--cti-text-primary' as string]: theme.textPrimary,
        ['--cti-text-secondary' as string]: theme.textSecondary,
        ['--cti-text-dim' as string]: theme.textDim,
        ['--cti-text-disabled' as string]: theme.textDisabled,
        ['--cti-bg-track' as string]: theme.bgTrack,
        ['--cti-bg-code' as string]: theme.bgCode,
        ['--cti-tab-border' as string]: theme.tabActiveBorder,
      } as React.CSSProperties}
    >
      {/* ── Top Bar: Header + Tab Navigation ── */}
      <div style={{
        background: `linear-gradient(180deg, ${theme.bgSecondary} 0%, ${theme.bgTertiary} 100%)`,
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '16px', flexWrap: 'wrap' }}>
            <div className="generatedAtBadge">
              <style>{`
                .generatedAtBadge {
                  padding: 8px 14px;
                  border-radius: 8px;
                  background: linear-gradient(135deg, var(--cti-accent-dim) 0%, var(--cti-accent-dim) 100%);
                  border: 1px solid var(--cti-accent-border);
                  position: relative;
                  animation: borderPulse 3s ease-in-out infinite;
                }
                @keyframes borderPulse {
                  0%, 100% { border-color: var(--cti-accent-border); box-shadow: 0 0 8px var(--cti-accent-dim); }
                  50% { border-color: var(--cti-accent-glow); box-shadow: 0 0 15px var(--cti-accent-dim); }
                }
                @keyframes cti-tab-in {
                ...
                .cti-tab-bar::-webkit-scrollbar { display: none; }
                @media (max-width: 520px) {
                  .generatedAtBadge { display: none !important; }
                }
                @media (prefers-reduced-motion: reduce) {
                  .generatedAtBadge { animation: none; }
                  [style*="cti-tab-in"] { animation: none !important; }
                }
              `}</style>
              <span style={{
                fontSize: '13px',
                ...
                {t('generated')}: <span style={{ color: 'var(--cti-text-primary)', fontWeight: 700 }}>
                  {new Date(data!.meta.generatedAt).toLocaleString(language === 'es' ? 'es-ES' : 'en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: '2-digit',
                    hour: '2-digit',
            ...
            <LanguageSwitcher />
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginLeft: '4px', paddingLeft: isMobile ? '8px' : '12px', borderLeft: `1px solid ${theme.borderPrimary}` }}>
              <a
                href="/"
                title="Home"
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  gap: '6px', padding: isMobile ? '8px' : '8px 14px',
                  borderRadius: '8px', textDecoration: 'none', minHeight: '36px',
                  background: 'var(--cti-bg-card)', border: '1px solid var(--cti-border)',
                  color: 'var(--cti-text-secondary)', fontSize: '0.85rem', fontWeight: 600,
                ...
                onMouseEnter={(e) => {
                  ...
                  el.style.borderColor = theme.borderSecondary;
                  el.style.color = theme.textPrimary;
                }}
                onMouseLeave={(e) => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.background = theme.bgCard;
                  el.style.borderColor = theme.borderPrimary;
        ...
        {/* Tab Bar */}
        <div className="cti-tab-bar" style={{
          display: 'flex',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isActive
                    ? 'linear-gradient(180deg, var(--cti-bg-track) 0%, var(--cti-bg) 100%)'
                    : 'transparent',
                  border: isActive
                    ? '1px solid var(--cti-tab-border)'
                    : '1px solid transparent',
                  borderBottom: isActive
                    ? '1px solid var(--cti-bg)'
                    : '1px solid transparent',
                  borderRadius: isMobile ? '6px' : '8px 8px 0 0',
                  color: isActive ? 'var(--cti-text-primary)' : 'var(--cti-text-dim)',
                  cursor: 'pointer',
                ...
                onMouseEnter={(e) => {
                  if (!isActive) {
                    (e.currentTarget as HTMLButtonElement).style.color = 'var(--cti-text-secondary)';
                    (e.currentTarget as HTMLButtonElement).style.background = 'var(--cti-bg-track)';
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isActive) {
                    (e.currentTarget as HTMLButtonElement).style.color = 'var(--cti-text-dim)';
              ...
              >
                <span style={{
                  fontSize: '14px',
                  color: isActive ? theme.accent : 'var(--cti-text-disabled)',
      ...
      {/* ── Panel Content ── */}
      <div key={activeTab} style={{ animation: 'cti-tab-in 0.18s ease-out', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
...
const MetricBox = ({ label, value, color, isMobile }: { label: string; value: number; color: string; isMobile?: boolean }) => (
  <div style={{
    ...
    <div style={{ fontSize: isMobile ? '9px' : '11px', color: 'var(--cti-text-muted)', textTransform: 'uppercase', marginTop: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      {label}
    </div>
  </div>
);
...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div style={{ marginBottom: '24px' }}>
    <div style={{
      ...
      marginBottom: '12px',
      paddingBottom: '8px',
      borderBottom: '1px solid var(--cti-tab-border)',
    }}>
      {title}
    </div>
    {children}
...
const LegendItem = ({ color, label, isMobile }: { color: string; label: string; isMobile?: boolean }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '3px' : '6px' }}>
    ...
    <span style={{ color: 'var(--cti-text-muted)', whiteSpace: 'nowrap' }}>{label}</span>
  </div>
);
